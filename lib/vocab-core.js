...
(function (root, factory) {
    ...
    const DEFAULT_SETTINGS = {
        ...
        frequency: 70,               // 0..100 — probability-like gate, deterministic per phrase
        replacementMode: 'replace',  // 'replace' | 'highlight' | 'beside'
        vieEngMode: true,            // match Vietnamese meanings -> show English
        engEngMode: false,           // match English synonyms -> show headword
        datasetKey: 'sat',
        contextCheckMode: 'off',     // 'off' | 'proxy' | 'byok'
        proxyUrl: ''                 // e.g. https://xxx.workers.dev
    };
     ...
     * @param {"vieEng"|"engEng"} mode
     */
    function buildVocabMap(activeVocab, mode) {
        const map = new Map();
    ...
    // ---------------------------------------------------------------------
    // Hashing + deterministic frequency gate
    // ---------------------------------------------------------------------

    /** Stable string hash (versioned so cache keys can be invalidated). */
    function simpleHash(str) {
        const versioned = 'v2|' + (str || '');
        let hash = 0;
        for (let i = 0; i < versioned.length; i++) {
            hash = ((hash << 5) - hash) + versioned.charCodeAt(i);
            hash |= 0;
        }
    ...
    // ---------------------------------------------------------------------
    // Context extraction + request building (small windows only — never pages)
    // ---------------------------------------------------------------------

    const MAX_CONTEXT_CHARS = 240;

    /** Token-radius window around a match (raw, may span sentence boundaries). */
    function extractContext(tokens, centerIdx, matchSize, radius) {
        radius = radius == null ? 15 : radius;
        const start = Math.max(0, centerIdx - radius);
        const end = Math.min(tokens.length, centerIdx + matchSize + radius);
        return tokens.slice(start, end).join('').trim();
    }

    /** Trim a context window down to the single sentence containing `matchedText`
     *  and cap its length. Keeps payloads to the AI small and page-content-free. */
    function toSentenceWindow(text, matchedText, maxLen) {
        maxLen = maxLen || MAX_CONTEXT_CHARS;
        ...
        if (chosen.length > maxLen) {
            // Center the window on the match when we have to truncate.
            const idx = matchedText ? chosen.toLowerCase().indexOf(matchedText.toLowerCase()) : 0;
            const half = Math.floor(maxLen / 2);
            const from = Math.max(0, (idx < 0 ? 0 : idx) - half);
            chosen = chosen.slice(from, from + maxLen).trim();
        }
        return chosen;
    }

    /** Normalized context signature used as a cache key (word blanked out). */
    function normalizeContext(context, word) {
        const words = (context || '').toLowerCase().split(/\s+/);
        const wordNorm = (word || '').toLowerCase();
        const idx = words.findIndex(w => w.includes(wordNorm));
        if (idx === -1) return (context || '').toLowerCase().slice(0, 60);
        const start = Math.max(0, idx - 3);
        const end = Math.min(words.length, idx + 4);
        return words.slice(start, end).join(' ').replace(wordNorm, '___');
    }

    /**
     * Build the minimal request item sent to the proxy/AI. Only a small sentence
     * window is included — never the full page.
     * @returns {{hash:string, vietnamesePhrase:string, candidateEnglish:string, sentenceContext:string, dataset:string}}
     */
    function buildContextRequestItem(params) {
        const matchedText = params.matchedText || '';
        const candidateEnglish = params.candidateEnglish || '';
        const sentenceContext = toSentenceWindow(params.context || '', matchedText);
        const contextPattern = normalizeContext(params.context || '', matchedText);
        const hash = params.hash || simpleHash(candidateEnglish.toLowerCase() + '|' + contextPattern);
        return {
            hash,
            vietnamesePhrase: matchedText,
            candidateEnglish,
            sentenceContext,
            dataset: params.dataset || 'SAT'
        };
    }

    /**
     * Parse a model's JSON reply ({"results":[{i,shouldReplace,confidence,reason}]})
     * into a hash -> {approved,confidence,reason} map. Tolerates code fences / extra
     * prose around the JSON. Any batch item the model didn't answer FAILS OPEN
     * (approved:true) so a flaky check never hides dataset words.
     * @param {string} rawText
     * @param {Array<{hash:string}>} batch
     */
    function parseModelResults(rawText, batch) {
        const out = {};
        let text = rawText || '';
        try {
            ...
            (parsed.results || []).forEach(r => {
                ...
                if (item) out[item.hash] = {
                    approved: r.shouldReplace !== false,
        ...
        (batch || []).forEach(c => {
            if (out[c.hash] === undefined) out[c.hash] = { approved: true, confidence: 0, reason: 'parse-fallback' };
        });
        return out;
    }
    ...
    return {
        ...
        // hashing/gate
        simpleHash, hashToInt, gateByFrequency,
        // context/request
        MAX_CONTEXT_CHARS, extractContext, toSentenceWindow, normalizeContext, buildContextRequestItem, parseModelResults,
        // csv
